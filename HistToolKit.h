...
int DrawSingleHistOnCanvas(string canvasname, TH1F* hist, string drawoption="PE", bool logx=false, bool logy=true, bool isrectangle=true)
{
...
//  DrawCMEAndLumi(0.5, 0.7, "XX", "13 TeV") ;
  ...
  leg->SetTextSize(0.04) ;
  leg->SetBorderSize(0) ;
  if(drawopt1.find("HIST")!=string::npos)
    leg->AddEntry(hist1, hist1->GetTitle(), "L") ;
  else
    leg->AddEntry(hist1, hist1->GetTitle(), "LPE") ;
  if(drawopt2.find("HIST")!=string::npos)
    leg->AddEntry(hist2, hist2->GetTitle(), "L") ;
  else
    leg->AddEntry(hist2, hist2->GetTitle(), "LPE") ;
  if(drawopt3.find("HIST")!=string::npos)
